...
/**
 * Created by lhx on 2016/8/24 10:27
 *
 * @Description
 */
...
//    private static final String PRE_STREAM_PRESET_TITLE = "888good";
    ...
    private static final String EXPECTED_BASE_HLS_LIVEURL = "http://" + "pili-live-hls.live.l99.com" + "/" + HUB_NAME;
    ...
    public void testCreateStream() {
        try {
            ...
            System.out.println(stream.getCreatedAt());
            System.out.println(stream.getUpdatedAt());
            System.out.println(stream.getTitle());
            System.out.println(stream.getHubName());
            System.out.println(stream.getPublishSecurity());
        } catch (PiliException e) {
            e.printStackTrace();
        }
...
//            e.printStackTrace();
    ...
    public void testSaveAs() {
        ...
        try {
            ...
            SaveAsResponse res = mStream.saveAs(fileName, format, 1000, 3600);
            fail();
        } catch (PiliException e) {
...
//            res = stream.saveAs(fileName, format, start, end, "http://notifyurl");
            res = stream.saveAs(fileName, format, start, end);
            assertNotNull(res);
    ...
    @Test
    public void testToJsonString() throws PiliException {
        // the test case order cannot be guaranteed
        if (mStream == null) {
            prepareStream();
        }
        System.out.println(mStream.status());
    ...
    @Test
    public void testdeleteAllStreams() {
        try {
            StreamList streamList = mHub.listStreams();
            if (streamList != null) {
                for (Stream stream : streamList.getStreams()) {
    ...
    public void testgetStream() {
        try {
            Stream stream = mHub.getStream("z1.lifeix-live.35481");
            Status status = stream.status();
            System.out.println("k");
//            String res = stream.delete();
//            System.out.println(res);
            Stream stream2 = stream.enable();
